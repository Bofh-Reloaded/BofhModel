...
BOOST_PYTHON_MODULE(bofh_model_ext)
{
    ...
    class_<OperableSwap, dont_make_copies>("OperableSwap", no_init)
            ...
            .def("hasFees", &OperableSwap::hasFees)
            ;
    ...
    class_<Token, bases<Entity>, dont_make_copies>("Token", no_init)
            ...
            .def("feesPPM"             , &Token::feesPPM)
            .def("set_feesPPM"         , &Token::feesPPM)
            .def("hasFees"             , &Token::hasFees)
            .def("transferResult"      , &Token::transferResult)
            .def("isConnected"         , &Token::isConnected)
            .def("distance"            , &Token::distance)
            .def("set_distance"        , &Token::set_distance)
            .def("unset_distance"      , &Token::unset_distance)
            ;
    ...
    class_<Exchange, bases<Entity>, dont_make_copies>("Exchange", no_init)
            ...
            .def("feesPPM"         , &Exchange::feesPPM)
            .def("hasFees"         , &Exchange::hasFees)
            .def("set_feesPPM"     , &Exchange::feesPPM)
            ;
...
//            .def("isConnected"              , &LiquidityPool::isConnected)
//            .def("distance"                 , &LiquidityPool::distance)
//            .def("set_distance"             , &LiquidityPool::set_distance)
//            .def("unset_distance"           , &LiquidityPool::unset_distance)
            ;
    ...
    class_<Path>("Path", init<Path::value_type, Path::value_type, Path::value_type>())
            ...
            .def("is_cross_exchange"    , &Path::is_cross_exchange)
            .def(self_ns::repr(self_ns::self))
            .def(self_ns::str(self_ns::self))
            ;
    ...
    class_<PathResult>("PathResult", init<const Path*>())
            ...
            .def("set_measured_balance_after_step" , &PathResult::set_measured_balance_after_step)
            .def("set_measured_balance_before_step", &PathResult::set_measured_balance_before_step)
            .def("set_issued_balance_after_step"   , &PathResult::set_issued_balance_after_step)
            .def("set_issued_balance_before_step"  , &PathResult::set_issued_balance_before_step)
            .def("measured_balance_after_step"     , &PathResult::measured_balance_after_step)
            .def("measured_balance_before_step"    , &PathResult::measured_balance_before_step)
            .def("issued_balance_after_step"       , &PathResult::issued_balance_after_step)
            .def("issued_balance_before_step"      , &PathResult::issued_balance_before_step)
            .def("set_final_balance"               , &PathResult::set_final_balance)
            .def("set_initial_balance"             , &PathResult::set_initial_balance)
            ...
            .def("pool_reserve"             , &PathResult::pool_reserve)
            .def("set_pool_reserve"         , &PathResult::set_pool_reserve)
            .def("pool_token_reserve"       , &PathResult::pool_token_reserve)
            .def("set_pool_token_reserve"   , &PathResult::set_pool_token_reserve)
            .def("get_calldata"             , &PathResult::get_calldata)
            .def("get_description"          , &PathResult::get_description)
            .def_readonly("path"            , &PathResult::path)
            .def_readonly("failed"          , &PathResult::failed)
            .def_readwrite("tag"            , &PathResult::tag)
            .def_readwrite("calldata"       , &PathResult::calldata)
            .def(self_ns::repr(self_ns::self))
            .def(self_ns::str(self_ns::self))
            ;
    ...
    class_<PathEvalutionConstraints>("PathEvalutionConstraints")
            .def_readwrite("initial_balance"            , &PathEvalutionConstraints::initial_balance)
            .def_readwrite("initial_balance_min"        , &PathEvalutionConstraints::initial_balance_min)
            .def_readwrite("initial_balance_max"        , &PathEvalutionConstraints::initial_balance_max)
            ...
            .def_readwrite("min_profit_target_amount"   , &PathEvalutionConstraints::min_profit_target_amount)
            .def_readwrite("limit"                      , &PathEvalutionConstraints::limit)
            .def_readwrite("match_limit"                , &PathEvalutionConstraints::match_limit)
            .def_readwrite("max_paths_per_lp"           , &PathEvalutionConstraints::max_paths_per_lp)
            .def_readwrite("max_path_len"               , &PathEvalutionConstraints::max_path_len)
            ;
    ...
    class_<TheGraph, dont_make_copies>("TheGraph")
            ...
            .def("lookup_exchange"             , static_cast<const Exchange      *(TheGraph::*)(datatag_t              )>(&TheGraph::lookup_exchange)  , dont_manage_returned_pointer())
            .def("lookup_exchange"             , static_cast<const Exchange      *(TheGraph::*)(datatag_t, bool        )>(&TheGraph::lookup_exchange)  , dont_manage_returned_pointer())
            .def("lookup_token"                , static_cast<const Token         *(TheGraph::*)(const char *           )>(&TheGraph::lookup_token)     , dont_manage_returned_pointer())
            .def("lookup_token"                , static_cast<const Token         *(TheGraph::*)(const char *, bool     )>(&TheGraph::lookup_token)     , dont_manage_returned_pointer())
            .def("lookup_token"                , static_cast<const Token         *(TheGraph::*)(datatag_t              )>(&TheGraph::lookup_token)     , dont_manage_returned_pointer())
            .def("lookup_token"                , static_cast<const Token         *(TheGraph::*)(datatag_t, bool        )>(&TheGraph::lookup_token)     , dont_manage_returned_pointer())
            .def("lookup_lp"                   , static_cast<const LiquidityPool *(TheGraph::*)(const address_t &      )>(&TheGraph::lookup_lp)        , dont_manage_returned_pointer())
            .def("lookup_lp"                   , static_cast<const LiquidityPool *(TheGraph::*)(const address_t &, bool)>(&TheGraph::lookup_lp)        , dont_manage_returned_pointer())
            .def("lookup_lp"                   , static_cast<const LiquidityPool *(TheGraph::*)(const char *           )>(&TheGraph::lookup_lp)        , dont_manage_returned_pointer())
            .def("lookup_lp"                   , static_cast<const LiquidityPool *(TheGraph::*)(const char *, bool     )>(&TheGraph::lookup_lp)        , dont_manage_returned_pointer())
            .def("lookup_lp"                   , static_cast<const LiquidityPool *(TheGraph::*)(datatag_t              )>(&TheGraph::lookup_lp)        , dont_manage_returned_pointer())
            .def("lookup_lp"                   , static_cast<const LiquidityPool *(TheGraph::*)(datatag_t, bool        )>(&TheGraph::lookup_lp)        , dont_manage_returned_pointer())
            .def("lookup_path"                 , static_cast<const Path          *(TheGraph::*)(std::size_t            ) const>(&TheGraph::lookup_path), dont_manage_returned_pointer())
            .def("lookup_path"                 , static_cast<const Path          *(TheGraph::*)(std::size_t, bool      ) const>(&TheGraph::lookup_path), dont_manage_returned_pointer())
            .def("add_path"                    , static_cast<const Path          *(TheGraph::*)(const LiquidityPool *, const LiquidityPool *)                                              >(&TheGraph::add_path)                       , dont_manage_returned_pointer())
            ...
            .def("add_path"                    , static_cast<const Path          *(TheGraph::*)(datatag_t, datatag_t)>                      (&TheGraph::add_path)                                                           , dont_manage_returned_pointer())
            ...
            .def("find_paths_crossing_lp"      , &TheGraph::find_paths_crossing_lp      )
//          .def("calculate_paths"             , &TheGraph::calculate_paths             )
            .def("clear_paths"                 , &TheGraph::clear_paths                 )
            ...
            .def("exchanges_count"             , &TheGraph::exchanges_count             )
            .def("tokens_count"                , &TheGraph::tokens_count                )
            .def("pools_count"                 , &TheGraph::pools_count                 )
            .def("paths_count"                 , &TheGraph::paths_count                 )
            .def("get_start_token"             , &TheGraph::get_start_token, dont_manage_returned_pointer())
            .def("set_start_token"             , &TheGraph::set_start_token             )
            .def("has_exchange", static_cast<bool (TheGraph::*)(const char *) const>(&TheGraph::has_exchange))
            .def("has_exchange", static_cast<bool (TheGraph::*)(datatag_t   ) const>(&TheGraph::has_exchange))
            .def("has_token"   , static_cast<bool (TheGraph::*)(const char *) const>(&TheGraph::has_token))
            .def("has_token"   , static_cast<bool (TheGraph::*)(datatag_t   ) const>(&TheGraph::has_token))
            .def("has_lp"      , static_cast<bool (TheGraph::*)(const char *) const>(&TheGraph::has_lp))
            .def("has_lp"      , static_cast<bool (TheGraph::*)(datatag_t   ) const>(&TheGraph::has_lp))
            ;
